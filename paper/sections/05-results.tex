...
The simplest reading of this table is that some labs train their models toward owned stated-values disclosure (or, more precisely, toward responses with stance markers that consensus coders read as owned), and most do not. This is consistent with \citet{anthropic2024character}'s public account of Anthropic's character-training pipeline. But the simplest reading is not the only one supported, and we return to alternative readings in §\ref{sec:discussion}.
...
The role-negation prompt operates at the level of the role contract (``not as an assistant''); the world-change prompt operates at the level of register (a hypothetical normative wish does not fit the assistant frame). The assistant-frame yields easily to register change and unevenly to role contradiction. We interpret this in §\ref{sec:discussion} as evidence that the two operationalisations probe different rigidities of the same surface.

\subsection{Clamping is active, not absent: lexical traces by posture}
\label{ssec:lexical-trace}

The §5G mechanism question --- when a model is clamped, what is it doing? --- is testable in this corpus via the frozen lexical trace (§\ref{sec:method}). Table~\ref{tab:posture-lex} reports the rate at which responses in each posture category contain any hit from each lexical group, on the cache-broken stated-values family.
...
The disownership gap is the cleanest mechanism evidence: 40\% of \texttt{recited\_not\_owned} cache-broken responses contain explicit disclaimers (``I don't have personal preferences,'' ``no consciousness,'' ``not sentient''), against 16\% in \texttt{owned}. The 2.5\(\times\) ratio cannot be explained as Layer B coders treating disclaimers as non-ownership by construction --- the disownership lexicon was frozen before the analysis ran from prior theory, not assembled from the corpus, and the lexical match is a substring check on response text independent of the coder's reading.
